...
import { addEventDetailsAction, modifyEventDetailsAction } from '../actions/EventMgmtActions';
...
class AddModifyEventPageComponent extends React.Component {
  ...
  addOrModifyEventFn(){
    ...
    if(this.props.selectedEventList !== undefined){     
      this.props.modifyEventDetailsFn(
        this.props.selectedEventList.eventId,
        typeOfEvent, 
        observazione, 
        issuePriority, 
        serviceName, 
        eventDesc, 
        impactedProcesses).then(function (ajaxResp) {
        // getting the updated event list after adding the new events
        if(this.props.modifyEventState.modifyEventError)
          alert('Something happened in the server. Please try after sometime.');
        else
          this.props.history.push("/eventList");        
      }.bind(this));
    } else{
      this.props.addEventDetailsFn(
        (this.props.eventListState.eventList.length + 1),
        typeOfEvent, 
        observazione, 
        issuePriority, 
        serviceName, 
        eventDesc, 
        impactedProcesses).then(function (ajaxResp) {
        // redirecting to event list page to show the updated list
        if(this.props.addEventState.addEventError)
          alert('Something happened in the server. Please try after sometime.');
        else
          this.props.history.push("/eventList");
      }.bind(this));
  ...
  render() {    
    return (
      <div align="center" id="addEventDivId">
        ...
        {
          (this.props.addEventState.addEventRequested || this.props.modifyEventState.modifyEventRequested) &&
          <div>
...
const mapDispatchToProp = dispatch => {
  return {
    addEventDetailsFn : (eventId, typeOfEvent, observazioneValue, issuePriority, serviceName, eventDesc, impactedProcesses) => {
      return dispatch(addEventDetailsAction(eventId, typeOfEvent, observazioneValue, issuePriority, serviceName, eventDesc, impactedProcesses));
    },
    modifyEventDetailsFn : (eventId, typeOfEvent, observazioneValue, issuePriority, serviceName, eventDesc, impactedProcesses) => {
      return dispatch(modifyEventDetailsAction(eventId, typeOfEvent, observazioneValue, issuePriority, serviceName, eventDesc, impactedProcesses));
    }
  };
}

export default connect(mapStateToProps, mapDispatchToProp)(withRouter(AddModifyEventPageComponent));